...
- fuel_monthly_current_year — помісячно за поточний календарний рік, без параметрів. Для «за цей рік» / «поточний рік» помісячно — лише цей інструмент. Інший рік помісячно в чаті недоступний (обмеження API); запропонуй команду /year у боті.
- fuel_recent_refuels(limit) — limit цілим числом, наприклад 15 (не рядок).
